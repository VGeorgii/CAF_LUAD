...
clin_df$dead = clin_df$vital_status == "Dead"


clin_df$overall_survival = ifelse(clin_df$dead,
                                  clin_df$days_to_death,
                                  clin_df$days_to_last_follow_up)


fit = survfit(Surv(overall_survival, dead) ~ status, data=clin_df, )

#plotting surv
ggsurvplot(fit, data=clin_df, pval=T, risk.table=T, palette = c("#7CAE00", "#00BFC4", "#F8766D"))
...
GO_results = enrichGO(gene = genes_to_test, keyType = "ENSEMBL", OrgDb = 'org.Hs.eg.db', ont = 'BP', pvalueCutoff=0.05,)
